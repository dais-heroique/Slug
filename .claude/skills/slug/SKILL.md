...
| Tool | Input | Result |
|------|-------|--------|
| `slug_snapshot` | `{ "scope": "focused" \| "window" \| "desktop", "filter"?: "send", "roles"?: ["button"], "interactive_only"?: true, "limit"?: 1, "coords"?: false }` (default scope `window`) | UI as Playwright-style YAML tree; each node has a short `[ref=…]`. Not a screenshot. **With `filter`/`roles`/`interactive_only` it returns a compact, LEAN flat list of just the matches (`role "name" [ref]` — coords omitted on normal controls). `roles` accepts exact names or groups (`clickable`/`field`/`text`/`link`/`heading`); matches are ranked exact-first so `limit:1` returns the one you meant; `coords:true` adds `@x,y` for a click fallback. The server-side "grep" that avoids shipping the whole 80k-char tree.** |
